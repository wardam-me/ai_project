...
class RecommendationSystem:
    ...
    def _get_security_focus(self):
        ...
        return max(security_concerns, key=security_concerns.get)
    ...
    def _get_network_preferences(self):
        ...
        if security_preference and max(security_preference.values()) > 0:
            preferred_security = max(security_preference, key=security_preference.get)
    ...
    def _get_recommended_actions(self):
        ...
        if low_security_networks:
            for network in set(low_security_networks)[:2]:  # Limiter à 2 recommandations
                recommended_actions.append({
