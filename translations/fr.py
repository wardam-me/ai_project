"""
Fichier de traduction française
"""

translations = {
    # Textes généraux
    "app_name": "Analyseur WiFi",
    "app_description": "Analyse en temps réel des réseaux disponibles",
    "error_not_found": "Page non trouvée. Retournez à l'accueil.",
    "error_server": "Erreur serveur. Veuillez réessayer plus tard.",
    
    # Navigation
    "nav_home": "Accueil",
    "nav_security_report": "Rapports de Sécurité",
    "nav_assistant": "Assistant",
    "nav_language": "Langue",
    "nav_recommendations": "Recommandations",
    "nav_device_security": "Sécurité Appareils",
    
    # Page d'accueil
    "best_network": "Meilleur Réseau Recommandé",
    "all_networks": "Tous les Réseaux Disponibles",
    "no_networks": "Aucun réseau détecté pour le moment...",
    "signal": "Signal",
    "frequency": "Fréquence",
    "security": "Sécurité",
    "security_unknown": "Inconnue",
    "analyze_security": "Analyser la sécurité",
    "consult_assistant": "Consulter l'assistant",
    "auto_update": "Les données sont mises à jour automatiquement toutes les 10 secondes.",
    "ssid": "SSID",
    "signal_strength": "Force du signal",
    
    # Rapports de sécurité
    "security_reports": "Rapports de Sécurité Réseau",
    "generate_report": "Générer un rapport",
    "report_name": "Nom du rapport (optionnel)",
    "report_name_placeholder": "Mon rapport de sécurité",
    "btn_generate_report": "Générer un rapport",
    "no_networks_detected": "Aucun réseau WiFi détecté.",
    "return_home": "Retournez à l'accueil pour détecter des réseaux.",
    "back_home": "Retour à l'accueil",
    "about_reports": "À propos des rapports",
    "reports_analyze": "Les rapports de sécurité analysent les réseaux WiFi selon plusieurs critères:",
    "encryption_type": "Type de chiffrement (WEP, WPA, WPA2, WPA3)",
    "signal_strength_criteria": "Force du signal",
    "frequency_used": "Fréquence utilisée",
    "security_score": "Chaque réseau reçoit un score de sécurité et des recommandations personnalisées.",
    "previous_reports": "Rapports précédents",
    "report_count": "rapport(s)",
    "report_date": "Date",
    "networks": "Réseaux",
    "actions": "Actions",
    "view": "Voir",
    "no_reports": "Aucun rapport de sécurité généré.",
    
    # Détails du rapport
    "report_summary": "Synthèse du rapport",
    "networks_analyzed": "Réseaux analysés",
    "secure_networks": "Réseaux sécurisés",
    "high_security": "Sécurité élevée",
    "vulnerable_networks": "Réseaux vulnérables",
    "low_security": "Sécurité faible",
    "general_recommendations": "Recommandations générales",
    "security_distribution": "Distribution de sécurité",
    "networks_details": "Détails des réseaux analysés",
    "security_score_text": "Score de sécurité",
    "network_info": "Informations réseau",
    "detailed_assessment": "Évaluation détaillée",
    "back_to_reports": "Retour aux rapports",
    
    # Assistant
    "assistant_title": "Assistant de Sécurité WiFi",
    "new_conversation": "Nouvelle conversation",
    "ask_security": "Posez une question sur la sécurité WiFi...",
    "select_network": "Aucun réseau",
    "send": "Envoyer",
    "greeting": "Bonjour ! Je suis votre assistant de sécurité WiFi. Comment puis-je vous aider aujourd'hui ?",
    "now": "Maintenant",
    "suggestions": "Suggestions de questions",
    "suggested_questions": [
        "Comment améliorer la sécurité de mon réseau WiFi ?",
        "Quelle est la différence entre WPA2 et WPA3 ?",
        "Comment créer un mot de passe WiFi sécurisé ?",
        "Est-ce que mon signal WiFi est assez fort ?",
        "Comment protéger mon réseau contre les pirates ?",
        "Quels sont les risques d'un réseau non sécurisé ?"
    ],
    "error_message": "Désolé, une erreur s'est produite lors de la communication avec l'assistant.",
    
    # Sécurité des appareils
    "device_security_title": "Sécurité des Appareils",
    "device_security_subtitle": "Analysez et surveillez la sécurité de tous les appareils connectés à votre réseau",
    "network_security_overview": "Aperçu de la Sécurité Réseau",
    "global_security_score": "Score de Sécurité Global",
    "critical_devices": "Appareils Critiques",
    "warning_devices": "Appareils à Risque",
    "secure_devices": "Appareils Sécurisés",
    "devices_list": "Liste des Appareils",
    "device_type": "Type d'Appareil",
    "manufacturer": "Fabricant",
    "security_score": "Score de Sécurité",
    "last_seen": "Dernière Activité",
    "refresh_devices": "Rafraîchir la Liste",
    "check_security": "Vérifier",
    "rec_device_secure": "Cet appareil est correctement sécurisé",
    "rec_update_firmware": "Mettre à jour le firmware de l'appareil",
    "rec_install_updates": "Installer les mises à jour de sécurité",
    "rec_address_vulnerabilities": "Corriger les vulnérabilités connues",
    "rec_close_ports": "Fermer les ports inutiles",
    "rec_improve_encryption": "Renforcer le chiffrement",
    "rec_enable_password": "Activer ou renforcer la protection par mot de passe",
    "rec_enable_firewall": "Activer ou configurer le pare-feu",
    "rec_check_suspicious": "Vérifier l'activité suspecte détectée",
    "refresh": "Rafraîchir",
    "security_refresh_message": "Les informations sont automatiquement mises à jour toutes les 30 secondes",
    "connected_devices": "Appareils Connectés",
    "no_devices_found": "Aucun appareil trouvé",
    "no_devices_message": "Aucun appareil n'a été détecté sur votre réseau pour le moment",
    "how_scoring_works": "Comment fonctionne la notation de sécurité",
    "scoring_explanation": "Notre système analyse chaque appareil connecté à votre réseau et attribue un score de sécurité basé sur plusieurs facteurs dont la version du firmware, les ports ouverts et les protocoles de sécurité actifs.",
    "critical_score": "Score Critique",
    "warning_score": "Score à Risque",
    "secure_score": "Score Sécurisé",
    "critical_score_description": "Nécessite une attention immédiate. Des vulnérabilités majeures ont été détectées.",
    "warning_score_description": "Des problèmes de sécurité modérés ont été identifiés et doivent être corrigés.",
    "secure_score_description": "L'appareil respecte les bonnes pratiques de sécurité et présente peu ou pas de risques.",
    "security_scoring_privacy": "Toutes les analyses sont effectuées localement. Aucune donnée n'est partagée avec des serveurs externes.",
    "device_details": "Détails de l'Appareil",
    "device_info": "Informations sur l'Appareil",
    "type": "Type",
    "first_seen": "Première Détection",
    "security_issues": "Problèmes de Sécurité",
    "issue": "Problème",
    "status": "Statut",
    "impact": "Impact",
    "security_recommendations": "Recommandations de Sécurité",
    "close": "Fermer",
    "recheck_device": "Revérifier",
    "devices_found": "{count} appareils trouvés",
    "details": "Détails",
    "unknown": "Inconnu",
    "critical": "Critique",
    "warning": "À risque",
    "secure": "Sécurisé",
    "firmware_version": "Version du Firmware"
}
